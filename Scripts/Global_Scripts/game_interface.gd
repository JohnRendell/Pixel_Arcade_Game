...
func _ready():
	...
	#TODO: do something on this, try and catch
	BackendStuff.get_data_from_backend("/gameData/getPlayerCount")
		
	await get_tree().create_timer(1.0).timeout
	if BackendStuff.returned_parsed["message"] == "success":
		playerCount.text = "Player Count: " + str(BackendStuff.returned_parsed["playerCount"])
...
func player_count(data):
	if typeof(data) == TYPE_DICTIONARY:
		if data.get("Socket_Type") == "playerCount":
			print("Get data from player count: " + str(data.get("Player_Count")))
			playerCount.text = "Player Count: " + str(data.get("Player_Count"))
